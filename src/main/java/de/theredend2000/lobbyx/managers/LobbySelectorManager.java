...
public class LobbySelectorManager {
    ...
    public void createLobbySelector(Player player){
        ...
        for(World world : plugin.getLobbyWorlds()){
            inventory.addItem(new ItemBuilder(plugin.getMaterial("LobbySelector.Material")).setDisplayname(plugin.getConfig().getString("LobbySelector.Name").replaceAll("%WORLD_NAME%", world.getName()).replaceAll("&","§")).setLore("§7Player: §9§l"+world.getPlayers().size(),plugin.getConfig().getBoolean("Lobby_Worlds.maintenance") ? "§4MAINTENANCE-MODE §cThis world is currently disabled" : "§eClick to warp.",player.hasPermission(Objects.requireNonNull(plugin.getConfig().getString("Permissions.MaintenanceModsPermission"))) ? "§7Right-Click to configure this world." : null).setLocalizedName(world.getName()).build());
        }
